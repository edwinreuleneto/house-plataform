      ...
      <div className="flex items-center justify-between h-full px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center space-x-4">
          <Button
            ...
            icon={sidebarOpen && isMobile ? <X className="h-5 w-5 text-black" /> : <Menu className="h-5 w-5 text-black" />}
          />
        </div>
          ...
          {/* <DropdownMenu> */}
            <div className="pv-4">
              <div className="flex items-center gap-1">
                <Avatar className="h-9 w-9 lg:h-9 lg:w-9" alt={getUserInitials(user?.user.name)} initials={user?.user.name.charAt(0)} />
                <div className="hidden lg:flex flex-col items-start w-[160px] ml-1">
                  <p className="text-sm font-semibold text-slate-800 truncate">{user?.user.name}</p>
